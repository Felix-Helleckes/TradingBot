...
def near_bb_lower(prices: List[float], period: int = BB_PERIOD, k: float = BB_K,
...
def load_data(pair: str, since_ts: int, end_ts: int, use_1h: bool) -> Dict[int, float]:
    if use_1h:
        ...
        candidates = sorted(DATA_15M.glob(f"{pair}_*_15m.json"),
...
def run_backtest(
    ...
    if not available:
        err = ("No 1h cache found. Expected data/mentor_cache_1h/*_60m.json"
               if use_1h else
               "No 15m data. Run: python3 scripts/collect_15m_daytrading.py --days 14")
        return {"error": err}
    ...
    for i, ts in enumerate(all_ts):
        ...
        eq_now = cash + sum(
            ...
            for p in available if pos[p].qty > 0
        )
        ...
        for p in available:
            ...
            if hit_tp or hit_sl or hit_time or hit_rsi:
                ...
                reason = ("TP" if hit_tp else
                          "RSI_EXIT" if hit_rsi else
                          "SL" if hit_sl else "TIME")
                closed.append({"pair": p, "pnl_eur": pnl_eur, "reason": reason,
                               "held_h": round(held_h, 1), "exit_rsi": round(rsi or 0, 1)})
    ...
    return {
        ...
        "params": {
            ...
            "max_hold_h": max_hold_h, "cooldown_min": cooldown_min,
            "atr_tp_mult": atr_tp_mult, "combo_mode": combo_mode,
...
def sweep(days: int, initial: float, fee: float, slip: float,
          use_1h: bool, combo: bool) -> None:
    if use_1h:
        configs = [
    ...
    for c in configs:
        ...
        print(f"{label:<48} {r['return_pct']:>+7.2f}% {r['winrate_pct']:>4.0f}% "
...
def main():
    ...
    if args.use_1h:
        if args.days   == 8:    args.days = 60
        if args.max_hold_h == 3.0: args.max_hold_h = 18.0
        if args.tp     == 1.2:  args.tp = 2.0
        if args.sl     == 0.8:  args.sl = 1.0
        if args.cooldown == 30: args.cooldown = 180

    if args.sweep:
        sweep(args.days, args.initial, args.fee, args.slippage_bps,
              use_1h=args.use_1h, combo=args.combo)
        return

    result = run_backtest(
        args.days, args.initial, args.fee, args.slippage_bps,
        buy_rsi=args.buy_rsi, sell_rsi=args.sell_rsi,
        tp_pct=args.tp, sl_pct=args.sl,
        max_hold_h=args.max_hold_h, cooldown_min=args.cooldown,
        use_1h=args.use_1h, combo_mode=args.combo,
    )
