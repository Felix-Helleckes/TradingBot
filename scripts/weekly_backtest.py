...
# Base directory where yearly subfolders (2025, 2026, ...) live
BASE = '/mnt/fritz_nas/Volume/kraken'
OUTDIR = '/home/felix/tradingbot/reports'
now = datetime.datetime.utcnow().strftime('%Y%m%dT%H%M%SZ')
OUT = os.path.join(OUTDIR, f'weekly_backtest_{now}.json')
...
# Gather symbols from all year subfolders under BASE
all_symbols = []
for year in sorted(os.listdir(BASE)):
    year_path = os.path.join(BASE, year)
    if not os.path.isdir(year_path):
        continue
    all_symbols.extend(find_symbol_dirs(year_path))
...
for sym, path in all_symbols:
    ...
    target_bars = 90*24*4  # 90 days of 15m bars
