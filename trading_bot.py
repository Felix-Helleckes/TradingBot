...
class TradingBot:
    ...
    def _journal_trade(self, ttype, pair, volume, price, pnl_eur, reason, extra=None):
        """Journal a trade: append a CSV line and a JSONL line (locked/atomic where possible).

        CSV write and JSONL write are separated into independent try/except blocks to
        avoid nested try complexity and ensure best-effort logging.
        """
        ...
        # JSONL structured journaling (use locked append helper)
        try:
            os.makedirs(os.path.dirname(self.json_journal_path), exist_ok=True)
            j = {
                'ts': datetime.datetime.utcnow().isoformat(),
